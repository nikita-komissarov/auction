<?php
  ...
  $media_list = [];
  if($item->media) $media_list = json_decode($item->media, true);

  $dir = 'media/items/'.$_POST['item'].'/';
  $upload_file = $_FILES['file'];
  $file_ext = mime2ext(mime_content_type($upload_file['tmp_name']));

  $result = yandexObjectStoragePut($upload_file['tmp_name'], $dir, count($media_list) ?: 0);

  $media_list[count($media_list)] = YANDEX_CLOUD_STORAGE['endpoint'].'/'.YANDEX_CLOUD_STORAGE_BUCKET.'/'.$result;
